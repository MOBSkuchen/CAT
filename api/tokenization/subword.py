...
class SubTokenizer(object):
  ...
  def decode(self, subtoken_ids):
    ...
    for token in escaped_tokens:
      # `token` may be empty string '':  'a_'.splitr('_') = ['a', ''] 
      if token:
...
def _escape_token(token, alphabet):
  ...
  Note that we assume `token` is a string containing either all alphanumeric 
  chars (i.e. `_ALPHANUMERIC_CHAR_SET`), or non-alphanumeric chars, but not 
  both. 

  Intuitively, at the time of decoding we want to tell exactly where each token 
  (generated by `_split_string_to_tokens`) ends. So when encoding, we add a 
  special char "_" at the end of each token, and we replace those "_" that 
  already exist in the token (if any) with a different char, "\\u". Also, for 
  the chars "c" in `token` that are not in the alphabet, we replace them with 
  the string "\ord(c);". 
...
def _unescape_token(token):
  ...
  Note: the trailing '_' has already been stripped from `token`.
  ...
  while True:
    ...
    if m is None:
      break
    token = re.subn(_OOA_CHAR_REGEX, chr(int(m.group(1))), token, count=1)[0]
  return token
